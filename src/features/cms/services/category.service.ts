...
export async function updateCategoryForTenant(input: {
  ...
  if (category.tenantId !== input.tenantId) {
    return { error: "Akses ditolak: Kategori bukan milik kelas Anda." } as const;
  }
  ...
  await updateCategory(input.id, { name: input.name, type: input.type });
...
export async function deleteCategoryForTenant(id: string, tenantId: string) {
  ...
  const inUse = await countCategoryTransactions(id);
  if (inUse > 0) {
