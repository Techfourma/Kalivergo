...
export default function PlatformLandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0a0a14] relative overflow-hidden text-white">
      <Loading
        isVisible={isLoading}
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#0a0a14]/80 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-8 text-sm text-gray-300">
              <button onClick={() => handleNavClick("#fitur")} className="hover:text-white transition-colors">
                Fitur
              </button>
              <button onClick={() => handleNavClick("#cara-kerja")} className="hover:text-white transition-colors">
                Cara Kerja
              </button>
              <button onClick={() => handleNavClick("#keunggulan")} className="hover:text-white transition-colors">
                Keunggulan
              </button>
            </div>
            ...
            <div className="hidden md:flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-medium text-gray-400 hover:text-white transition-all"
              >
              ...
              <button
                ...
                className="rounded-xl border border-white/20 bg-white/5 px-4 py-2 text-sm font-medium text-white hover:bg-white/10 transition-all"
              >
                Masuk
              </button>
              <button
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-white/10 bg-[#0a0a14]/95 backdrop-blur-md px-4 py-4 space-y-1">
            {[
              ...
              <button
                ...
                className="block w-full text-left px-3 py-2 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors"
              >
                {item.label}
              </button>
            ))}
            <div className="flex gap-3 pt-3">
              <button
                onClick={() => handleNavClick("/login")}
                className="flex-1 rounded-xl border border-white/20 bg-white/5 px-4 py-2 text-sm font-medium hover:bg-white/10 transition-all"
              >
                Masuk
              </button>
              <button
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <section className="py-24 lg:py-32 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-10">
              <button
                onClick={() => handleNavClick("/signup")}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary-500 to-accent-500 px-8 py-3.5 text-base font-semibold text-white hover:shadow-2xl hover:shadow-primary-500/40 hover:scale-105 transition-all"
              >
                Daftarkan Kelasmu
              ...
              {FEATURES.map((feature) => {
                ...
                return (
                  <div
                    key={feature.title}
                    className={cn(
                      "rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 p-6 transition-all hover:scale-[1.02]",
                      feature.border
                    )}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {STEPS.map((step) => {
                ...
                return (
                  <div
                    key={step.step}
                    className="relative rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 p-6 hover:bg-white/10 transition-all"
                  >
                    <span className="absolute top-5 right-6 text-4xl font-bold font-display text-white/10">
                      {step.step}
                    </span>
...
{/* Benefits */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
              {BENEFITS.map((benefit) => {
                ...
                return (
                  <div
                    key={benefit.title}
                    className="flex items-start gap-4 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 p-6 hover:bg-white/10 transition-all"
                  >
          ...
          <section className="py-20">
            <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-primary-600/30 via-dark-800/40 to-accent-600/30 p-10 sm:p-16 text-center">
      ...
      {/* Footer */}
      <footer className="border-t border-white/10 py-8 bg-[#0a0a14]/90 backdrop-blur-md relative z-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
