...
import { CalendarDays, Save, Wallet, ChevronDown, ChevronUp } from "lucide-react";
...
export default function UangKasSettingsCard({ dates = [], amount = 10000 }: UangKasSettingsCardProps) {
  ...
  const activeDates = billingDates.filter(Boolean);
  const displayAmount = Number(cashAmount).toLocaleString("id-ID");
          ...
          <div className="text-left">
            ...
            <p className="text-sm text-dark-500 dark:text-dark-400 mt-0.5">
              {isExpanded
                ...
                : `${activeDates.length} tanggal tagihan · Nominal Rp ${displayAmount}`}
            </p>
          </div>
        </div>
        <div className="shrink-0 ml-4 text-dark-400">
