  ...
  return (
    <Card>
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          <h2 className="text-lg font-bold text-dark-900">Pendataan Uang Kelas</h2>
      ...
      >
        <div className="md:col-span-2">
          <label className="mb-1.5 block text-sm font-medium text-dark-700">Tanggal tagihan</label>
          <div className="space-y-2">
            {billingDates.map((date, index) => (
              <div key={`${index}-${date}`} className="flex items-center gap-2">
                <div className="relative flex-1">
                  <CalendarDays className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-dark-400" />
                  <input
                    type="date"
                    name="dates"
                    value={date}
                    ...
                    required
                    className="w-full rounded-xl border border-dark-200 py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                  />
                </div>
                {billingDates.length > 1 && (
                  <button type="button" onClick={() => setBillingDates((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="px-2 text-sm text-red-600">Hapus</button>
                )}
              </div>
            ))}
            <button type="button" onClick={() => setBillingDates((current) => [...current, ""])} className="text-sm font-medium text-primary-600 hover:text-primary-700">+ Tambah tanggal tagihan</button>
          </div>
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-dark-700">Nominal uang kas</label>
          <input
            type="number"
            name="amount"
            min="1"
