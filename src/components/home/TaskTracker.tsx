  ...
  return (
    <Card padding="lg">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-dark-900">Task Tracker</h2>
        ...
        <div className="flex items-end gap-2">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-black-600 tracking-widest ml-1 mb-1">TGL. MULAI</span>
            <input
              type="date"
              value={dateRange.start}
              onChange={(e) => setDateRange({...dateRange, start: e.target.value })}
              className="rounded-lg border border-dark-200 px-3 py-1.5 text-sm text-dark-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
          <span className="text-dark-400 pb-2">-</span>
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-black-600 tracking-widest ml-1 mb-1">TGL. SELESAI</span>
            <input
              type="date"
              value={dateRange.end}
        ...
        {filteredTasks.length === 0? (
          ...
          filteredTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className="group rounded-xl border border-dark-100 bg-dark-50/50 p-4 hover:border-primary-200 hover:bg-primary-50/30 transition-all duration-200"
              >
